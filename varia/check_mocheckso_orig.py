...
# VARS
DB_CONNECTION = 'host=srsofaioi4531.ktso.ch dbname=sogis user=datasync password=topsecret'
...
URL_FTP = 'ftp://xxxx:yyyyy@ftp.infogrips.ch/DM01AVSO24/ERROR/MOCHECKSO_ERRORS.CSV'
#URL_FTP = 'ftp://vaso:vaso123@ftp.infogrips.ch/DM01AVSO24/ERROR/MOCHECKSO_errors.CSV'
...
EMAIL_RECEIVER = 'andrea.luescher@bd.so.ch'
...
def load_csv(db, import_table, csv_file):
    ...
    try:
        icmd = '/usr/local/gdal/bin/ogr2ogr --config PGCLIENTENCODING LATIN1 -f "PostgreSQL" PG:"' + db +'" '+ csv_file + ' -overwrite -nln ' + import_table
        os.system(icmd)
    except Exception, e:
...
def update_table(cur, tmp_table_name, table_name='mocheckso_script_import_test'):
    ...
    CASE WHEN errorx::text = 'NULL' AND errory::text = 'NULL' THEN
        ST_PointFromText('POINT('||'0'||' '||'0'||')', 21781)
    WHEN errorx::text = 'NULL' AND NOT errory::text = 'NULL' THEN
        ST_PointFromText('POINT('||'0'||' '||errory::text||')',21781)
    WHEN NOT errorx::text = 'NULL' AND errory::text = 'NULL' THEN
        ST_PointFromText('POINT('||errorx::text||' '||'0'||')',21781)
    ELSE
        ST_PointFromText('POINT('||errorx::text||' '||errory::text||')',21781)
    END AS wkb_geometry
    FROM %s;
    DROP TABLE %s;
    commit;
...
def check_csv(cur, csv_file, email_sender, email_receiver):
    ...
    for line in dictContent: # for each line in the csv-dictionary object
        ...
        if rows[0][0] < 1: # if count < 1 warning
            arrErrorString.append('<b>BFS-number not accurate: </b>' + line['DatasetID'])
